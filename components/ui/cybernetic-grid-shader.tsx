  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // base grid color pulsing - using purple theme
        vec3 gridColor = vec3(0.5, 0.2, 1.0); // Purple instead of blue
        vec3 color = gridColor * line * (0.3 + sin(t * 2.0) * 0.1);
        
        // energetic pulses along grid
        float energy = sin(uv.x * 20.0 + t * 5.0) * sin(uv.y * 20.0 + t * 3.0);
        energy = smoothstep(0.8, 1.0, energy);
        color += vec3(0.8, 0.4, 1.0) * energy * line * 0.5; // Purple energy
        
        // glow around mouse
        float glow = smoothstep(0.1, 0.0, mouseDist);
        color += vec3(0.6, 0.3, 1.0) * glow * 0.3; // Purple glow
        
        // subtle noise
        color += random(uv + t * 0.1) * 0.02;
        
        gl_FragColor = vec4(color, 0.6); // Reduced opacity for background
      }
    `;
